<?php 
    ...
    if ( ! function_exists('tgl_indo_format'))
    {
    ...
    //Long date indo Format
    if ( ! function_exists('longdate_indo_v2'))
    {
        function longdate_indo_v2($tanggal)
        {
            $ubah = gmdate($tanggal, time()+60*60*8);
            $pecah = explode("/", $ubah);
            $tgl = $pecah[0];
    ...
    if ( ! function_exists('jamServer'))
    {
        function jamServer($datetime)
        {
            $jam = substr($datetime,10,6);
            return $jam;
        }
    }   
    ...
    function formatTanggal($datetime) {
        // Membuat objek DateTime dari string datetime SQL
